...
try
    ...
    % v = VideoReader('hp_small.mp4');    
    % iseq = read_video_sequence(VideoStruct);
    % VE = get_video_events(VideoStruct);

    ShotStartEnd = videoevents_to_shots(VideoStruct, 5);
    
    % ShotStartEnd = videoevents_to_shots(iseq, VE);
    keyframes = round(mean(ShotStartEnd, 2));
    num_shots = length(keyframes);

    v = VideoReader('/scratch/jaidev/hp_full_small.mp4');
    
    meanrgbhist = zeros(num_shots, params.descriptor.rgbhist.bins3d^3, 1);
